...
namespace WinterboltGames.BillboardGenerator.Runtime.Renderers
{
	...
	public class SimpleBillboardRenderer : BillboardRendererBase
	{
		...
		public override void UpdateBillboard()
		{
			...
			var direction = cachedTransform.position - targetPosition;

			var rawAngle = Mathf.Atan2(direction.x, direction.z) * Mathf.Rad2Deg;
		...
		public override void RenderBillboard() => Graphics.DrawMesh(mesh, Matrix4x4.TRS(cachedTransform.position, cachedTransform.rotation, cachedTransform.lossyScale), material, gameObject.layer);
	}
}
