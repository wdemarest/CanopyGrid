...
namespace WinterboltGames.BillboardGenerator.Runtime.Renderers
{
	...
	public sealed class DirectionalBillboardRenderer : BillboardRendererBase
	{
		...
		public override void PrepareBillboard()
		{
			...
			for (var i = 0; i < 360 / _snap; i++) _directions.Add(i * _snap, i);
		}
		...
		public override void UpdateBillboard()
		{
			...
			var direction = cachedPosition - targetPosition;

			var rawAngle = Mathf.Atan2(direction.x, direction.z) * Mathf.Rad2Deg;

			var parentAngle = (cachedTransform.parent == null ? 0.0f : cachedTransform.parent.eulerAngles.y) % 360.0f;

			parentAngle = parentAngle < 0.0f ? parentAngle + 360.0f : parentAngle;

			var angle = Mathf.CeilToInt(rawAngle - parentAngle) / _snap * _snap % 360;

			if (angle < 0) angle += 360;

			if (_directions.TryGetValue(angle != 0 && angle != 360 ? angle : 0, out var index))
			{
				var uvs = atlas.uvCoordinates[index];
		...
		public override void RenderBillboard() => Graphics.DrawMesh(mesh, Matrix4x4.TRS(cachedTransform.position, cachedTransform.rotation, GetScale()), material, gameObject.layer);
		...
		private Vector3 GetScale(bool lossy = true) => lossy ? Vector3.Scale(atlas.rendererScale, cachedTransform.lossyScale) : Vector3.Scale(atlas.rendererScale, localScale);
	}
}
