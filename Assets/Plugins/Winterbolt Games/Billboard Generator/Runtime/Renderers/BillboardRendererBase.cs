...
namespace WinterboltGames.BillboardGenerator.Runtime.Renderers
{
	...
	public abstract class BillboardRendererBase : MonoBehaviour
	{
		...
		{
			if (useBillboardTarget && target != BillboardTarget.TargetTransform) target = BillboardTarget.TargetTransform;
		...
		protected virtual Mesh MakeQuad()
		{
			...
			var vertices = new[] { new Vector3(-0.5f, -0.5f), new Vector3(0.5f, -0.5f), new Vector3(0.5f, 0.5f), new Vector3(-0.5f, 0.5f), };

			mesh.vertices = vertices;

			var triangles = new[] { 2, 1, 0, 3, 2, 0, };

			mesh.triangles = triangles;

			var uv = new[] { new Vector2(0.0f, 0.0f), new Vector2(1.0f, 0.0f), new Vector2(1.0f, 1.0f), new Vector2(0.0f, 1.0f), };
