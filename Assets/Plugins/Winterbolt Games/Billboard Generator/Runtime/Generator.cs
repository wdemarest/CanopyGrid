	...
	public static class Generator
	{
		...
		private static void Prepare(in RawTransform transform, in GeneratorSettings settings)
		{
			if (settings.useMainCamera)
			{
				...
				{
					var mainCameraClone = Object.Instantiate(Camera.main);
			...
			else
			{
				...
				else _camera.fieldOfView = settings.cameraSizeOrFieldOfView;

				var position = transform.TransformPoint(settings.cameraOffset);
		...
		public static Texture2D[] Generate(in RawTransform transform, in GeneratorSettings settings)
		{
			try
			{
				...
				var angle = 360 / settings.directions;

				var textures = new Texture2D[settings.directions];

				for (var i = 0; i < settings.directions; i++)
				{
					_camera.Render();

					var texture = new Texture2D(settings.textureWidth, settings.textureHeight);

					texture.ReadPixels(new Rect(0.0f, 0.0f, settings.textureWidth, settings.textureHeight), 0, 0);

					texture.Apply();

					var result = transform.RotateTransform(new RawTransform(_camera.transform), transform.Up, angle);
		...
		public static void CaptureSingleShotAsync(in RawTransform transform, in GeneratorSettings settings, string outputFolderPath)
		{
			try
			{
				...
				var renderTexture = RenderTexture.GetTemporary(width, width);
		...
		private static void CleanUp()
		{
			try
			{
				...
				if (_camera != null)
				{
					...
					else Object.DestroyImmediate(_cameraGameObject);
