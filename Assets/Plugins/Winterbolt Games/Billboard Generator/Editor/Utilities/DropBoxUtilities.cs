...
namespace WinterboltGames.BillboardGenerator.Editor.Utilities
{
	internal static class DropBoxUtilities
	{
		...
		private static T[] DropBoxHandler<T>(Rect rect, Action<T[]> onDrop = null) where T : Object
		{
			...
			if (!(currentEvent.type == EventType.DragUpdated || currentEvent.type == EventType.DragPerform)) return new T[0];

			if (!rect.Contains(currentEvent.mousePosition)) return new T[0];

			var objectReferences = DragAndDrop.objectReferences.OfType<T>().ToList();

			DragAndDrop.visualMode = DragAndDropVisualMode.Copy;

			if (currentEvent.type != EventType.DragPerform) return new T[0];
