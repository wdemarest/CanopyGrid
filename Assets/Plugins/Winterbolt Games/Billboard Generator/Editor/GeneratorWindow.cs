...
namespace WinterboltGames.BillboardGenerator.Editor
{
	internal sealed class GeneratorWindow : EditorWindow
	{
		...
		private List<GeneratorWindowEntry> _entries = new List<GeneratorWindowEntry>();
		...
		private void OnGUI()
		{
			...
			{
				foreach (var gameObject in gameObjects) _entries.Add(new GeneratorWindowEntry { gameObject = gameObject, });
			});
		...
		{
			var targetTransform = new RawTransform(targetGameObject.transform);

			var defaultScenePath = string.Empty;
			...
			if (settings.isolateTarget)
			{
				...
				var temporaryScene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Additive);

				foreach (var clone in Instantiate(targetGameObject).transform.GetTree())
				{
				...
				if (EditorUtility.DisplayDialog("Confirm", "Bake Lighting?", "Bake Lighting", "Do Not Bake Lighting")) _ = Lightmapping.Bake();
			}

			try
			{
				var textures = Generator.Generate(targetTransform, settings);

				(var texture, var uvs) = PackingUtilities.PackTight(textures, settings.freeRectChoiceHeuristic, settings.initialBinWidth, settings.initialBinHeight);
			...
			if (settings.isolateTarget) EditorSceneManager.OpenScene(defaultScenePath, OpenSceneMode.Single);
		}
