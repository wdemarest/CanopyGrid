...
namespace WinterboltGames.BillboardGenerator.Editor
{
	...
	internal sealed class GeneratorSettingsAssetInspector : UnityEditor.Editor
	{
		...
		{
			var settingsProperty = serializedObject.FindProperty(nameof(GeneratorSettingsAsset.settings));
			...
			var useMainCameraProperty = settingsProperty.FindPropertyRelative(nameof(GeneratorSettings.useMainCamera));
			...
			if (!useMainCameraProperty.boolValue)
			{
				...
				var useOrthographicCameraProperty = settingsProperty.FindPropertyRelative(nameof(GeneratorSettings.useOrthographicCamera));
			...
			else EditorGUILayout.HelpBox("Some options are disabled when 'Use Main Camera' is enabled.", MessageType.Info, true);
			...
			var isolateTargetProperty = settingsProperty.FindPropertyRelative(nameof(GeneratorSettings.isolateTarget));

			if (isolateTargetProperty.boolValue && useMainCameraProperty.boolValue) EditorGUILayout.HelpBox("'Use Main Camera' MUST be disabled when 'Isolate Target' is enabled otherwise the rendering is going to fail or give 100% undesirable results.", MessageType.Warning, true);
		...
		public override void OnInspectorGUI() => Draw(serializedObject);
	}
}
