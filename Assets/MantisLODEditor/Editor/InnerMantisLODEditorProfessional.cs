...
namespace MantisLODEditor
{
    ...
    {
        private MantisMeshList mantisMeshList = new MantisMeshList();

        private MantisParams param = new MantisParams() { protect_boundary = true, protect_shape = true };
        private MantisParams save = new MantisParams() { protect_boundary = true, protect_shape = true };

        private bool show_title = true;
        private bool show_help = true;
        private bool save_show_help = true;

        private MantisStandard standard = new MantisStandard();
        private MantisProgressive progressive = new MantisProgressive();

        void init_all()
        {
            if (target)
            {
                bool facesCounted = mantisMeshList.init_all(((Component)target).gameObject);
                if (facesCounted)
                {
        ...
        void clean_all()
        {
            if (target)
            {
                mantisMeshList.clean_all();
            }
        }
        void optimize()
        {
            mantisMeshList.optimize(param);
        }
        override public void OnInspectorGUI()
        {
            void refresh()
            {
                param.quality = 100.0f;
                ...
                if (!mantisMeshList.isEmpty)
                {
                    ...
                    if (show_title)
                    {
                        ...
                        if (GUILayout.Button("Save Progressive Mesh", GUILayout.ExpandWidth(true), GUILayout.ExpandWidth(true)))
                        {
                            ...
                            ProgressiveMesh pm = progressive.Create(mantisMeshList);
                        ...
                        {
                            optimize();
                            var filePathList = new Dictionary<Mantis_Mesh, string>();
                            mantisMeshList.Traverse((Mantis_Mesh child) =>
                            {
                                string filePath = EditorUtility.SaveFilePanelInProject(
                                    "Save Current Mesh",
                                    child.mesh.name + "_Quality_" + param.quality.ToString() + ".asset",
                                    "asset",
                                    "Choose a file location"
                                    );
                                if (filePath != "")
                                {
                                    filePathList[child] = filePath;
                                }
                            });

                            mantisMeshList.Traverse((Mantis_Mesh child) =>
                            {
                                standard.Save(child.mesh, filePathList[child]);
                            });
                        }
                        if (show_help)
                        {
                        ...
                        progressive.max_lod_count = EditorGUILayout.IntField("Max LOD Count", progressive.max_lod_count);
                        if (show_help)
                        {
                            GUILayout.Label(
                        ...
                        param.protect_boundary = EditorGUILayout.Toggle("Protect Boundary", param.protect_boundary);
                        if (show_help)
                        {
                            GUILayout.Label(
                        ...
                        param.protect_detail = EditorGUILayout.Toggle("More Details", param.protect_detail);
                        if (show_help)
                        {
                            GUILayout.Label(
                        ...
                        param.protect_symmetry = EditorGUILayout.Toggle("Protect Symmetry", param.protect_symmetry);
                        if (show_help)
                        {
                            GUILayout.Label(
                        ...
                        param.protect_normal = EditorGUILayout.Toggle("Protect Hard Edge", param.protect_normal);
                        if (show_help)
                        {
                            GUILayout.Label(
                        ...
                        param.protect_shape = EditorGUILayout.Toggle("Beautiful Triangles", param.protect_shape);
                        if (show_help)
                        {
                            GUILayout.Label(
                        ...
                        param.use_detail_map = EditorGUILayout.Toggle("Use Detail Map", param.use_detail_map);
                        if (show_help)
                        {
                            GUILayout.Label(
                        ...
                        if (param.use_detail_map)
                        {
                            param.detail_boost = EditorGUILayout.IntField("Detail Boost", param.detail_boost);
                            if (show_help)
                            {
                                GUILayout.Label(
                        ...
                        EditorGUILayout.LabelField("Triangles", mantisMeshList.face_number.ToString() + "/" + mantisMeshList.origin_face_number.ToString());
                        if (show_help)
                        {
                            GUILayout.Label(
                        ...
                        param.quality = EditorGUILayout.Slider("Quality", param.quality, 0.0f, 100.0f);
                        if (show_help)
                        {
                            GUILayout.Label(
                        ...
                        if (GUI.changed)
                        {
                            ...
                            if (save.protect_boundary != param.protect_boundary)
                            {
                                refresh();
                                save.protect_boundary = param.protect_boundary;
                            }
                            if (save.protect_detail != param.protect_detail)
                            {
                                refresh();
                                save.protect_detail = param.protect_detail;
                            }
                            if (save.protect_symmetry != param.protect_symmetry)
                            {
                                refresh();
                                save.protect_symmetry = param.protect_symmetry;
                            }
                            if (save.protect_normal != param.protect_normal)
                            {
                                refresh();
                                save.protect_normal = param.protect_normal;
                            }
                            if (save.protect_shape != param.protect_shape)
                            {
                                refresh();
                                save.protect_shape = param.protect_shape;
                            }
                            if (save.use_detail_map != param.use_detail_map)
                            {
                                refresh();
                                save.use_detail_map = param.use_detail_map;
                            }
                            if (save.detail_boost != param.detail_boost)
                            {
                                refresh();
                                save.detail_boost = param.detail_boost;
                            }
                            if (save.quality != param.quality)
                            {
                                mantisMeshList.optimize(param);
                                if (mantisMeshList.set_quality(param.quality))
                                {
                                    EditorUtility.SetDirty(target);
                                }
                                save.quality = param.quality;
                            }
                        }
                    }
                }
...
{
    [CustomEditor(typeof(MantisLODEditorProfessional))]
    public class OLD___InnerMantisLODEditorProfessional : Editor
    {
        // run when focused
        [DllImport("MantisLOD")]
